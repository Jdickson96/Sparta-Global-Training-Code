...
namespace Methods_Lib
{
    public class Methods
    {
        ...
        public static (int weeks, int days) DaysAndWeeks(int totalDays)
        {
            ...
            else return (totalDays / 7, totalDays % 7);
        }

        public static (int square, int cube, double squareRoot) PowersRoot (int input)
        {
            if(input < 0) throw new ArgumentOutOfRangeException("Input Number must not be negative");
            else  return (input*input, input * input * input, Math.Sqrt(input));
        }
